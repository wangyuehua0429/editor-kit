...
- `v0.3` —— 5 平台 × 4 取向专用 prompt 体系（20 个文件）+ 调试面板取向联动 + 全部 prompt 表述修复
- `v0.3.1` —— 全局 prompt 优化：输出禁区全覆盖 + 风格段多段化 + 平台×取向专项适配（80+ 次实测驱动）
- `v0.3.2` —— 微博 4 取向 prompt 优化：修中性版缺段 + 短版 140 字硬约束 + 长版"沿用细节+改写语气"定位 + 短/长版去重 + 数据加和/改单位/模糊化禁止 + 短版必含事实要素 + 删模板括号占位说明
